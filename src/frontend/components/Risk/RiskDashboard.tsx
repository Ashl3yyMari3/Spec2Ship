...
import criticalPlanet from '../../assets/cosmic/critical-planet.png';
import highRiskPlanet from '../../assets/cosmic/high-risk-planet.png';
import mediumRiskPlanet from '../../assets/cosmic/medium-risk-planet.png';
import lowRiskPlanet from '../../assets/cosmic/low-risk-planet.png';

const RISK_CARD_IMAGES: Record<RiskTier, string> = {
  Critical: criticalPlanet,
  High: highRiskPlanet,
  Medium: mediumRiskPlanet,
  Low: lowRiskPlanet,
};
...
function tierOrbClass(tier: RiskTier): string {
  switch (tier) {
    case 'Critical':
      return 'risk-score-orb--critical';
    case 'High':
      return 'risk-score-orb--high';
    case 'Medium':
      return 'risk-score-orb--medium';
    case 'Low':
      return 'risk-score-orb--low';
    default:
      return '';
  }
}
...
function tierReqCardClass(tier: RiskTier): string {
  switch (tier) {
    case 'Critical':
      return 'risk-req-card risk-req-card--critical';
    case 'High':
      return 'risk-req-card risk-req-card--high';
    case 'Medium':
      return 'risk-req-card risk-req-card--medium';
    case 'Low':
      return 'risk-req-card risk-req-card--low';
    default:
      return 'risk-req-card';
  }
}
...
function getRawValue(riskScore: RiskScore, key: string): number {
  switch (key) {
    case 'criticality':
      return riskScore.factors.criticalityRaw;
    case 'coverageGap':
      return riskScore.factors.coverageGapRaw;
    case 'executionCompleteness':
      return riskScore.factors.executionCompletenessRaw;
    case 'changeImpact':
      return riskScore.factors.changeImpactRaw;
    default:
      return 0;
  }
}
...
function getWeightedContribution(
  ...
  switch (key) {
    case 'criticality':
      return riskScore.weightedContributions.criticality;
    case 'coverageGap':
      return riskScore.weightedContributions.coverageGap;
    case 'executionCompleteness':
      return riskScore.weightedContributions.executionCompleteness;
    case 'changeImpact':
      return riskScore.weightedContributions.changeImpact;
    default:
      return 0;
  }
}
...
function RiskScoreOrb({
  ...
  return (
    <div
      className={`risk-score-orb ${tierOrbClass(tier)}`}
    ...
    >
      ...
      <div className="risk-score-orb__inner">
        <span className="risk-score-orb__value">
          {score}
        </span>
      </div>
    </div>
...
// --- Individual Risk Requirement Card ---
...
export default function RiskDashboard({
  ...
  const sorted = [...scores].sort((a, b) => {
    ...
    return a.requirementId.localeCompare(
      b.requirementId,
    );
  });
    ...
    <section aria-label="Risk dashboard">
      ...
      <div className="risk-kpi-grid">
        {(
            ...
            style={{
              backgroundImage: `
                linear-gradient(
                  ...
                  rgba(5, 8, 22, 0.18) 0%,
                  rgba(5, 8, 22, 0.28) 55%,
                  rgba(5, 8, 22, 0.48) 100%
                ),
                url(${RISK_CARD_IMAGES[tier]})
              `,
          ...
          >
            <p className="risk-kpi-card__value">
              {tierCounts[tier]}
            </p>

            <p className="risk-kpi-card__label">
              {tier} Risk
            </p>
      ...
      {/* Requirement risk cards */}
      {sorted.map((riskScore) => (
        <RiskCard
          key={riskScore.requirementId}
          riskScore={riskScore}
          requirement={reqMap.get(
            riskScore.requirementId,
          )}
        />
      ))}
